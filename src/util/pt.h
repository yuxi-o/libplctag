...
#define PT_FUNC_DECLARE(name, ...)  \                                  \
static int name(FOREACH_PAIR_LAST(PT_MAKE_FUNC_ARG, PT_MAKE_FUNC_ARG_LAST, __VA_ARGS__));\
...
int name(FOREACH_PAIR_LAST(PT_MAKE_FUNC_ARG, PT_MAKE_FUNC_ARG_LAST, __VA_ARGS__)) {\
    struct pt_args_##name *pt_args = mem_alloc(sizeof(struct pt_args_##name));\
    if(!pt_args) return -1; \
...
#define PT_CONTEXT(...) \
    ...
    if(!ctx) { \
        ctx = mem_alloc(sizeof(struct pt_ctx));                        \
